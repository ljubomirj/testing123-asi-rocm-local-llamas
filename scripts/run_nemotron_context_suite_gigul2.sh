...
MODEL_ALIAS="${MODEL_ALIAS:-nemotron-cascade-2-30b-a3b}"
...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/nemotron_gigul2_context_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$RUN_ROOT"
...
run_case() {
  ...
  if [[ "$mode" == "off" ]]; then
    args+=(--assistant-prefill '<think></think>')
...
for mode in off on; do
  start_server "$mode"
  if [[ "$mode" == "off" ]]; then
    backend="llamacpp-hip-rocwmma-wrapper-10t-nemotron-iq4xs"
    suffix=""
  else
    backend="llamacpp-hip-rocwmma-wrapper-10t-nemotron-iq4xs-thinking-on"
    suffix="_thinking_on"
  fi

  run_case "$mode" "none" 0 "50,100" "$NONE_MAX_TOKENS" \
    "$RUN_ROOT/benchmark_nemotron_cascade_2_30b_iq4_xs_none_wrapper_10t${suffix}.jsonl" \
    "$backend"
  run_case "$mode" "small" 5000 "10000,15000" "$CONTEXT_MAX_TOKENS" \
    "$RUN_ROOT/benchmark_nemotron_cascade_2_30b_iq4_xs_small_5k_wrapper_10t${suffix}.jsonl" \
    "$backend"
  run_case "$mode" "mid" 20000 "10000,15000" "$CONTEXT_MAX_TOKENS" \
    "$RUN_ROOT/benchmark_nemotron_cascade_2_30b_iq4_xs_mid_20k_wrapper_10t${suffix}.jsonl" \
    "$backend"
  run_case "$mode" "long" 40000 "10000,15000" "$CONTEXT_MAX_TOKENS" \
    "$RUN_ROOT/benchmark_nemotron_cascade_2_30b_iq4_xs_long_wrapper_10t${suffix}.jsonl" \
    "$backend"
  run_case "$mode" "longlong" 100000 "10000,15000" "$CONTEXT_MAX_TOKENS" \
    "$RUN_ROOT/benchmark_nemotron_cascade_2_30b_iq4_xs_longlong_100k_wrapper_10t${suffix}.jsonl" \
    "$backend"
