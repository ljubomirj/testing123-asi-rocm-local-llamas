...
def main():
    parser = argparse.ArgumentParser(
        description="A/B correctness + timing for int8 KV MLA decode kernel"
    )
    parser.add_argument(
        "--config",
        type=Path,
    ...
    kernel_ms = dt * 1000.0 / args.iters

    # Reference
    k_groups = qk_head_dim // kv_group_size
    v_groups = v_head_dim // kv_group_size
    k_deq = (
        k_cache[:seq_len]
        .float()
        .view(seq_len, kv_head_num, k_groups, kv_group_size)
        * k_scale[:seq_len].float().unsqueeze(-1)
    ).view(seq_len, kv_head_num, qk_head_dim)
    v_deq = (
        v_cache[:seq_len]
        .float()
        .view(seq_len, kv_head_num, v_groups, kv_group_size)
        * v_scale[:seq_len].float().unsqueeze(-1)
    ).view(seq_len, kv_head_num, v_head_dim)

    ref = torch.empty_like(out, dtype=torch.float32)
    kv_group_num = head_num // kv_head_num
    for b in range(args.batch):
        t0 = int(kv_indptr[b].item())
        t1 = int(kv_indptr[b + 1].item())
        idx = kv_indices[t0:t1]
        for h in range(head_num):
            kv_head = h // kv_group_num
            K = k_deq.index_select(0, idx)[:, kv_head, :]
            V = v_deq.index_select(0, idx)[:, kv_head, :]
            q_vec = q[b, h].float()
            qk = (K * q_vec).sum(dim=1) * float(args.sm_scale)
            if args.logit_cap > 0.0:
                qk = args.logit_cap * torch.tanh(qk / args.logit_cap)
            p = torch.softmax(qk, dim=0)
            ref[b, h] = (p.unsqueeze(1) * V).sum(dim=0)

    max_abs = (ref - out.float()).abs().max().item()

    tokens = args.batch * seq_len
    tok_s = tokens / (dt / args.iters) if dt > 0 else 0.0
    print(
    ...
    print(f"kernel_ms={kernel_ms:.3f} tok/s={tok_s:.2f} max_abs_diff={max_abs:.6f}")
