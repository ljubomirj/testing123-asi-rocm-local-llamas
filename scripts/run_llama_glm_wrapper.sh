#!/bin/bash
# llama-server watchdog wrapper for GLM-4.7-Flash
# Monitors and automatically restarts the llama-server if it dies or becomes unresponsive
...
SERVER_PORT="8081"
MODEL_PATH="$HOME/llama.cpp/models/GLM-4.7-Flash-UD-Q4_K_XL.gguf"

# Health check interval (seconds)
HEALTH_CHECK_INTERVAL=300

# Server command arguments (matching the manual launch)
SERVER_ARGS=(
  --device ROCm0
  --gpu-layers all
  --ctx-size 130000
...
start_server() {
  ...
  local server_log
  server_log="$LLAMA_CPP_DIR/log_llama-server-glm-4.7-flash-ppid_$$_$(date +'%Y%m%d_%H%M%S').log"
  ...
  log "ACTION: Waiting for server to initialize (15 seconds)"
  sleep 15
